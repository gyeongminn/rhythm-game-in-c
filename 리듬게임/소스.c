...
char file[NUM_SONG][23] = { "GoodDay.txt" ,"NeverEndingStory.txt","Payphone.txt" };
...
void start_game(int song)
{
	...
	draw_map(song);
	// 노래 재생
	if (song == 0) PlaySound(TEXT("GoodDay.wav"), NULL, SND_ASYNC);
	else if (song == 1) PlaySound(TEXT("NeverEndingStory.wav"), NULL, SND_ASYNC);
	else PlaySound(TEXT("Payphone.wav"), NULL, SND_ASYNC);
	now = clock();
	while (1)
	{
		if (m > 0)
...
void write_note(int song)
{
	...
	clock_t now, now2;
	FILE* fp;
	if ((fp = fopen("note.txt", "w")) == NULL) // 맵파일 읽어오기
	{
		printf("맵파일을 열 수 없습니다.\n");
		return;
	}
